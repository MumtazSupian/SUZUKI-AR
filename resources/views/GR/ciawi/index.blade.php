                        ...
                        <tr style="background-color: {{ $loop->even ? '#fef2f2' : '#ffffff' }};">
                            ...
                            <td class="col-action">
                                <div style="display:flex; gap:6px; justify-content: center;">
                                    ...
                                    <form method="POST" action="{{ url('/gr/ciawi/' . ($row->id ?? ($row['id'] ?? ''))) }}"
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Listener Input Pencarian Real-time
        if (searchInput) {
            searchInput.addEventListener('input', function () {
                const filter = this.value.toLowerCase();
                const rows = document.querySelectorAll('#piutangTable tbody tr:not(.no-data-row)');
                ...
                // Menangani tampilan jika data kosong setelah di-filter
                const noDataRow = document.querySelector('.no-data-row');
                if (!hasVisibleRow) {
                    if (!noDataRow) {
                        const tbody = document.querySelector('#piutangTable tbody');
                        const newRow = document.createElement('tr');
                        newRow.className = 'no-data-row';
                        newRow.style.backgroundColor = '#ffffff';
                        newRow.innerHTML = `<td colspan="15" style="text-align:center; color: #6b7280 !important; padding: 20px;">Tidak ada data yang cocok dengan pencarian.</td>`;
