...
<div style="width: 100%; box-sizing: border-box; overflow-x: hidden;">
    <div class="page-header" style="margin-bottom: 20px;">
        <div>
            ...
            <p class="page-subtitle">Kelola data saldo awal, mutasi, rekonsiliasi GL, dan saldo akhir konsumen cabang Cinere.</p>
        </div>
                        ...
                        <tr>
                            ...
                            <td class="col-action">
                                <div style="display:flex; gap:6px;">
                                    <a href="{{ url('/gr/cinere/' . ($row->id ?? ($row['id'] ?? '')) . '/edit') }}" class="action-btn edit" title="Edit" style="text-decoration: none;">✎</a>
                                    <form method="POST" action="{{ url('/gr/cinere/' . ($row->id ?? ($row['id'] ?? ''))) }}" style="display:inline">@csrf @method('DELETE')<button class="action-btn delete" title="Hapus">🗑</button></form>
                                </div>
                            </td>
                        </tr>
                    @empty
